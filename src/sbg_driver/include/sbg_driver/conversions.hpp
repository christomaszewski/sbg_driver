...
// EkfNav.status carries no GNSS fix type of its own (only aiding bits), so the
// fix grade is borrowed from `gnss_pos_status` - the status word of the most
// recent PRIMARY-receiver GnssPos log - while the EKF reports GPS1 position
// aiding (SBG_ECOM_SOL_GPS1_POS_USED). status.status is therefore:
//   * STATUS_NO_FIX - position-valid bit clear
//   * to_navsat()'s grade (GBAS for RTK/PPP/DGPS, SBAS, FIX for single) -
//     position valid, GPS1 position used, and a GnssPos word is known
//   * STATUS_FIX - position valid but dead-reckoning (GPS1 not used), no
//     GnssPos word seen yet, or the word decodes below FIX
// `service` is left at the conventional SERVICE_GPS. Covariance diagonal is
// filled from EkfNav.positionStdDev² in ENU [east, north, up] order
// (frame-convention independent - NavSatFix is always geographic). Altitude is
// promoted from MSL to ellipsoidal height via undulation, matching to_navsat().
[[nodiscard]] std::unique_ptr<sensor_msgs::msg::NavSatFix> to_ekf_navsat(
  ...
  std::optional<std::uint32_t> gnss_pos_status = std::nullopt);
